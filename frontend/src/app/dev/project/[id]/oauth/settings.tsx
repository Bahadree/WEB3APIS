...
export default function OAuthSettings() {
  ...
  const fetchScopesAndApp = async () => {
    ...
    try {
      ...
      let localScopesRaw = typeof window !== "undefined" ? localStorage.getItem(LS_KEY) : null;
      if (localScopesRaw) {
        try {
      ...
      const projectRes = await fetch(`/api/dev/projects/${projectId}`, {
      ...
      if (!loadedScopes.length) {
        const scopesRes = await fetch(`/api/dev/projects/${projectId}/oauth-scopes`, {
          headers: { Authorization: `Bearer ${jwt}` },
        });
        const scopesData = await scopesRes.json();
        if (scopesData && Array.isArray(scopesData.scopes) && scopesData.scopes.length > 0) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/dev/projects/${projectId}/oauth-apps`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${jwt}` },
        body: JSON.stringify({
          appName: appName || "Default App",
          redirectUris: redirectUris.length > 0 ? redirectUris : ["http://localhost:3000"],
          scopes,
        }),
      });
  ...
  return (
    <>
      <div className="max-w-xl mx-auto my-12 bg-card border border-border rounded-2xl p-8 shadow">
        <h2 className="text-2xl font-bold mb-6 text-primary text-center">OAuth Ayarları</h2>
        <div className="flex flex-col items-center justify-center min-h-[300px]">
          <div className="mb-6 w-full max-w-md">
            <div className="mb-2 font-semibold">İzin Verilecek Bilgiler (Scopes):</div>
            <div className="space-y-3">
              {DEFAULT_SCOPES.map((scope) => (
                <label
                  key={scope.key}
                  className={`flex items-center gap-3 cursor-pointer select-none p-3 rounded-xl border transition-all duration-200 ${scopes.includes(scope.key) ? "border-primary bg-primary/10" : "border-border bg-muted hover:bg-accent/30"}`}
                >
                  ...
                  <span className="text-base font-medium text-foreground">{scope.label}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          {success && <div className="mt-4 text-green-600 text-center">Başarıyla kaydedildi.</div>}
